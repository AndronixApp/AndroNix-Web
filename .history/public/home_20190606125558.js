...
var uid; var io = false; var no = 0; var comNum;
//getComNumber();

document.getElementById('adder').addEventListener('click', adder);
document.getElementById('play').addEventListener('click', adder);
document.getElementById('github').addEventListener('click', adder);
document.getElementById('logout').addEventListener('click', adder);
...
//   var number;
//   // var numberRef = db.collection('numbers').doc(uid);
//   console.log("giveNumberOfCom is running");
//   var db = firebase.firestore();
//   var user = firebase.auth().currentUser;


//   db.collection('numbers').doc("eRPki1dTX2wHbIY6kN1F")
//     .onSnapshot(function (doc) {
//       comNum = doc.data().comNo;
//       number = doc.data().comNo;
//       console.log("comNum is " + comNum);
//       console.log("number is " + number);
//       document.getElementById("comNum").innerHTML = "You have " + number + " commands synced on the cloud. Nice work, happy coding!";
//     });
  ...
  if (com != null) {
    if (dis != null) {
      if (color != null) {
        ...
        window.alert("Please enter a command in the field.");
      }
    }
    else {
      window.alert("Please enter a description in the field.");
    }
  }
  else {
    window.alert("Please choose a color to code the cards.");
  }

}

// function updateComNumber(process) {
//   var db = firebase.firestore();
//   var numberRef = db.collection('numbers').doc("eRPki1dTX2wHbIY6kN1F");
//   if (process == "add") {
//     numberRef.update({
//       comNo: firebase.firestore.FieldValue.increment(1)
//     });

//     document.getElementById("comNum").innerHTML = "You have " + comNum + 1 + " commands synced on the cloud. Nice work, happy coding!";

//   }
//   else if (process == 'delete' && comNum >= 0) {
//     numberRef.update({
//       comNo: firebase.firestore.FieldValue.increment(-1)
//     });
